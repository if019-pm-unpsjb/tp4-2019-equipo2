...
int inicializar( int puerto ) {
	...
	if ( bind( lsock, (struct sockaddr *) &dir_srv, sizeof( dir_srv ) ) < 0 )
		return ( -2 );

	if ( bind( lsock_udp, (struct sockaddr *) &dir_srv, sizeof( dir_srv ) ) < 0 )
		return ( -2 );
